...
CKEDITOR.plugins.add( 'responsivevoice',
{
	...
	init: function( editor )
	{
		...
		editor.addCommand( 'cmdResponsiveVoice',
			{
				...
				exec : function( editor )
				{
					...
					responsiveVoice.speak(editor.document.getBody().getText(), 'US English Male');
				}
			});
		// Create a toolbar button that executes the plugin command.
		// http://docs.cksource.com/ckeditor_api/symbols/CKEDITOR.ui.html#addButton
		editor.ui.addButton( 'ResponsiveVoice',
		{
			// Toolbar button tooltip.
			label: 'ResponsiveVoice',
			// Reference to the plugin command name.
			command: 'cmdResponsiveVoice',
			// Button's icon file path.
			icon: this.path + 'images/speaker.png'
		} );
	}
} );